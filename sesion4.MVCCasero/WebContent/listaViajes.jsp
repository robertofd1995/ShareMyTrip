...
<script type="text/javascript">
	...
	function() {
	var $panel = $(this).parents('.filterable'), $filters = $panel
	.find('.filters input'), $tbody = $panel.find('.table tbody');
	...
	$('.filterable .filters input').keyup(function(e) {
		...
		if (code == '9')
			return;
		var $input = $(this), inputContent = $input.val().toLowerCase(),
	...
	if ($filteredRows.length === $rows.length) {
		$table.find('tbody').prepend(
			$('<tr class="no-result text-center"><td colspan="'+ $table.find('.filters th').length
